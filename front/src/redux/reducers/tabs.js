...
export default function (state = initialState, action) {
  const open = settingId => Object.assign({}, state, {
    ids: state.ids.includes(settingId) ? state.ids : state.ids.concat(settingId),
    activeId: settingId
  })
  const del = settingId => {
  ...
  const change = settingId => Object.assign({}, state, { activeId: state.ids.includes(settingId) ? settingId : state.activeId })
